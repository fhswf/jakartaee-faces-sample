 ...
 * from the page-scoped FacesContext to the session-scoped session map.</li>
 * <li>Before messages are rendered, this moves the messages from the
 * session-scoped session map back to the page-scoped FacesContext.</li> Only
 * messages that are not associated with a particular component are ever moved.
 * These are the only messages that can be rendered on a page that is different
